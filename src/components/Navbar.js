...
const Links = ['Home', 'About Us', 'Coalescence', 'Novatia', 'Initiatives', 'Contact'];
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <Flex pos="absolute" top="0" right="0" h='95px' alignItems={'center'} pr={4}>
          <IconButton
            variant='unstyled'
            color='#FDC20A'
            bg='black'
            size={'lg'}
            icon={isOpen ? 
            <HamburgerIcon boxSize={8} transform={'rotate(-90deg)'} transitionDuration={'0.8s'}/> : 
      ...
      {isOpen ? (
          <Slide direction='center' in={isOpen} style={{position: 'relative' }}>
            <Box fontWeight={700} fontSize='20px' bg='blackAlpha.700' display={{ md: 'none' }} pb={4} pr={8}>       
